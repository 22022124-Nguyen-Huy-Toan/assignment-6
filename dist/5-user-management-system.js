...
exports.setupFormHandling = setupFormHandling;
...
function formatUserInfoAlternative(user) {
...
try {
    ...
    console.log("Error in function test:", error.message);
}
// ========================================
...
for (let i = 0; i < 3; i++) {
    ...
    users.push(user);
    const userAccount = new UserAccount(user.id, user.username, user.email, `password${i + 1}23`, user.role, user.isActive);
...
const adminAccount = new AdminUser(adminUser.id, adminUser.username, adminUser.email, "supersecurepassword123", ["read", "write", "delete", "manage_users", "system_config"]);
...
const profileUsers = users.slice(0, 2).map(user => ({
    ...user,
...
try {
    ...
    console.log(`  Regular user short password: ${error.message}`);
}
try {
    ...
    console.log(`  Admin short password: ${error.message}`);
}
// Demo 5: Type guards và utility functions
console.log("\n5. Demonstrating type safety:");
const mixedUsers = [...users, ...profileUsers];
